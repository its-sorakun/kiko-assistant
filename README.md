...
- **Direct Memory Screen Reading**: Rips raw chat strings directly from physical RAM via a native C++ scanner to read active heavily-sandboxed applications (like Discord or WhatsApp Web) bypassing brittle OCR or UI automation.
- **Native Multimodal Vision**: Actually "sees" the game or screen natively via DXGI Desktop Duplication or GDI BitBlt, piping the raw GPU frame buffer directly into the Gemini Vision encoder.
- **Silent Filesystem Inspection**: Enables the assistant to query directory structures natively and ingest folder contents directly into its LLM context without relying on visual Windows Explorer windows. Path resolution natively supports the `~` alias shortcut.
- **Advanced Agentic PDF Reading**: Extracts text, chunks, and embeds massive PDFs using `HuggingFace` embeddings and `FAISS`. Utilizes `LangGraph` under the hood to act as an advanced Retrieval-Augmented Generation (RAG) sub-agent, entirely disconnected from external bloated frameworks for maximum CPU speed.
