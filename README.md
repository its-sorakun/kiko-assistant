...
- **Universal GUI Screen Reading**: Hooks into the native Microsoft UI Automation (UIA) API to crawl the accessibility trees of active web browsers or non-IDE GUI applications, enabling Kiko to "see" your screen natively.
...
- **Native Screen Reading & Direct Memory Hooking**: Extends Kiko's vision by hooking into the native Microsoft UI Automation (UIA) API to crawl accessibility trees of active web browsers. If UIA fails due to sandboxing (like in Discord or Electron apps), Kiko falls back to a custom C++ Direct Memory Scanner. This drops down to `ReadProcessMemory` to rip strings directly from the target application's active `PAGE_READWRITE` heap chunks, cross-sectioning the memory and stripping V8 Javascript noise to extract pure chat context.
