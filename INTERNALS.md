...
## 17. Native Bluetooth & WinRT Radio Management
**File:** `tools/bluetooth_mgr.py`, `tools/bt_manager/bt_manager.cpp`, `tools/bt_manager/ToggleBluetooth.ps1`

Rather than relying on brittle UI automation to manage Bluetooth pairings or radio states, Kiko interacts directly with the lowest levels of the Windows Bluetooth stack, bypassing abstracted UI elements.
- **Native Device Management (C++ `bluetoothapis.h`)**: A custom C++ binary (`bt_manager.exe`) directly invokes `BluetoothFindFirstDevice` and `BluetoothFindNextDevice` to list paired hardware natively. To force connections, Kiko bypasses standard pairing wrappers and drops a direct `BLUETOOTH_SERVICE_ENABLE` or `BLUETOOTH_SERVICE_DISABLE` flag onto the device's `GUID_AUDIO_SINK`. This kernel-level instruction aggressively triggers the Windows audio stack to connect or disconnect the headset without UI prompts.
- **WinRT Master Radio Toggle (`Windows.Devices.Radios`)**: Toggling the master hardware radio switch requires UWP AppContainer privileges on modern Windows builds. To safely proxy this boundary without crashing the Python interpreter, Kiko spawns an isolated PowerShell process. This script utilizes C#-style reflection to properly extract the `AsTask` method for `IAsyncOperation<T>`, bridging the WinRT `GetRadiosAsync` and `SetStateAsync` APIs. 
- **LTSC / UWP Privilege Bypassing (Capability Access Manager)**: Because the `Windows.Devices.Radios` API throws a fatal `DeniedByUser` exception for all Win32 unpackaged desktop apps by default, Kiko utilizes a profound OS capability bypass. She drops down into the Windows Registry (`HKLM\Software\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\radios`) to autonomously inject the global `Allow` permission flag prior to executing the Bluetooth payload. This forces the WinRT kernel to instantaneously authorize the unpackaged PowerShell proxy. Once the Master Radio state is flipped, Kiko meticulously restores the Registry value back to `Deny` (if it was originally disabled) to strictly enforce OS privacy.
- **Battery Query Limitations**: The raw Win32 API `BLUETOOTH_DEVICE_INFO` structurally omits battery life data. Querying battery requires WinRT GATT enumerators (`System.Devices.BatteryLife`), which heavily lag or fail on non-BLE devices. Thus, Kiko restricts enumeration to pure connectivity status to prioritize speed.

## 18. Master Capability Access Manager (Permission Controller)
**File:** `tools/permissions_handle.py`

Modern Windows permissions (Location, Radios, Camera, Microphone) are enforced by the Capability Access Manager Service (`camsvc`). Standard WinRT APIs (e.g., `AppCapability::RequestAccessAsync`) only allow an application to politely request permission for *itself* via a UI prompt, but Microsoft purposefully does not expose a public API to globally toggle the master OS switch programmatically.
